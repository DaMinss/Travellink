...
public class TripCloudAdapter extends RecyclerView.Adapter<TripCloudAdapter.TripViewHolder> implements Filterable {
    ...
    public TripCloudAdapter(List<TripDAO.Trip_withTotalPrice> _list, Context context) {
        listOfTrips = _list;
        listOfTrips1 = _list;
    ...
    public TripViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.trip_card_admin, parent, false);
    ...
    public void onBindViewHolder(@NonNull TripViewHolder holder, int position) {
        ...
        }else if (trip_withTotalPrice.getTrip().getTrip_status().equals("Approve")){
            holder.Trip_Status.setTextColor(Color.rgb(57, 148, 4 ));
            holder.Trip_Status.setText("Approve");
        }else if(trip_withTotalPrice.getTrip().getTrip_status().equals("Decline")){
            holder.Trip_Status.setTextColor(Color.rgb(57, 148, 4 ));
            holder.Trip_Status.setText(trip_withTotalPrice.getTrip().getTrip_status());
        }
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection("user").document(trip_withTotalPrice.getTrip().getUid())
                .get()
                .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                    ...
                    public void onSuccess(DocumentSnapshot documentSnapshot) {
                        if (documentSnapshot.exists()) {
                            ...
                            holder.userName.setText(userData.getEmail());
                        }
                    }
                });
    ...
    public class TripViewHolder extends RecyclerView.ViewHolder {
        public TextView Trip_Name,Trip_Price, Trip_StartDate,Trip_endDate, Trip_Depart,Trip_Arrival, to, Trip_Status, Status, userName;
